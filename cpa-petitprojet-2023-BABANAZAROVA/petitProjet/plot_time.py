...
"""def lire_frequences_et_tracer_graphique(chemin_fichier):
    ...
    # Par exemple, afficher une graduation tous les 5 ms
    plt.xticks(np.arange(min(tranches_triees), max(tranches_triees) + 5, 5.0))
...
def lire_frequences_et_tracer_graphique(chemin_fichier):
    ...
    # Créer un barplot
    sns.barplot(x=tranches_triees, y=counts_triees, ax=ax)

    # Personnaliser le graphique
    ax.set_xlabel('Temps (ms)')
    ax.set_ylabel('Fréquence')
    ax.set_title('Diagramme de fréquence des temps d\'exécution')

    # Ajuster les graduations de l'axe des abscisses
    # Par exemple, afficher une graduation tous les 5 ms
    valeurs_xticks = np.arange(min(tranches_triees), max(tranches_triees) + 5, 5.0)
    ax.set_xticks(valeurs_xticks)
    ax.set_xticklabels([str(v) for v in valeurs_xticks])
